...
import { getDeckLinkStats, isPlausibleConfigured, type LinkStats } from "@/lib/plausible";
 ...
 * for a single token. Thin wrapper over the batched path so the merge logic lives
 * in exactly one place.
 */
export async function getMergedStats(siteId: string, token: string): Promise<FullStats> {
  return (await getMergedStatsForTokens(siteId, [token]))[token];
}
